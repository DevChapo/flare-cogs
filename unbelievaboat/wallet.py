...
class Wallet(MixinMeta):
    ...
    async def bankdeposit(self, ctx, user, amount):
        ...
        try:
            ...
            msg = f"{user.display_name} has succesfully deposited {deposit} {await bank.get_currency_name(ctx.guild)} into their bank account."
    ...
    async def bankwithdraw(self, ctx, user, amount):
        ...
        try:
            ...
            return await ctx.send(
                f"{user.display_name} has succesfully withdrawn {humanize_number(amount)} {await bank.get_currency_name(ctx.guild)} from their bank account."
            )
        except ValueError:
